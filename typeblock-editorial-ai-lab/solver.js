...
function intrinsic(e,p){let width=Math.max(360,$('layout').clientWidth||720),cw=(width-G*5)/6,px=cw*p.span+G*(p.span-1),hp=p.rows*U,cpl=Math.max(8,px/7.15),lineCount=Math.max(2,Math.floor((hp-20-(e.cue?34:0))/20)),cap=Math.max(1,cpl*lineCount),fill=Math.min(1,e.chars/cap),lineCost=cpl<34?(34-cpl)*1.7:cpl>72?(cpl-72)*1.35:0,fillCost=fill<.55?(.55-fill)*155:fill<.72?(.72-fill)*24:0,aspect=px/hp,aspectCost=aspect<.72?(.72-aspect)*30:aspect>5.4?(aspect-5.4)*8:0;return{cost:clamp(lineCost+fillCost+aspectCost,0,100),fill,cpl,aspect}}
function spatialContext(p,state){let recent=state.ps.slice(-4),cost=20;if(recent.some(q=>q.x===p.x))cost-=10;if(recent.some(q=>q.x+q.span===p.x+p.span))cost-=5;if(p.x===0||p.x+p.span===6)cost-=5;if(recent.at(-1)?.span===p.span)cost+=7;return clamp(cost,0,100)}
...
function whiteField(ps,start,V){let g=Array.from({length:V},()=>Array(C).fill(0));ps.forEach(p=>{let y0=Math.max(start,p.row),y1=Math.min(start+V,p.row+p.rows);for(let y=y0;y<y1;y++)for(let x=p.x;x<p.x+p.span;x++)g[y-start][x]=1});let seen=Array.from({length:V},()=>Array(C).fill(false)),comps=[],dirs=[[1,0],[-1,0],[0,1],[0,-1]];for(let y=0;y<V;y++)for(let x=0;x<C;x++){if(g[y][x]||seen[y][x])continue;let q=[[x,y]],k=0,cells=[],minx=x,maxx=x,miny=y,maxy=y,lat=false;seen[y][x]=true;while(k<q.length){let[cx,cy]=q[k++];cells.push([cx,cy]);minx=Math.min(minx,cx);maxx=Math.max(maxx,cx);miny=Math.min(miny,cy);maxy=Math.max(maxy,cy);if(cx===0||cx===C-1)lat=true;dirs.forEach(([dx,dy])=>{let nx=cx+dx,ny=cy+dy;if(nx>=0&&nx<C&&ny>=0&&ny<V&&!g[ny][nx]&&!seen[ny][nx]){seen[ny][nx]=true;q.push([nx,ny])}})}let bw=maxx-minx+1,bh=maxy-miny+1,box=bw*bh;comps.push({area:cells.length,bw,bh,lat,ragged:box?1-cells.length/box:0})}comps.sort((a,b)=>b.area-a.area);let total=C*V,largest=comps[0]||{area:0,bw:0,bh:0,lat:false,ragged:0},largestRatio=largest.area/total,small=comps.filter(c=>c.area/total<.03),slivers=comps.filter(c=>(c.bw===1&&c.bh>=V*.18)||(c.bh<=2&&c.bw>=3)),white=comps.reduce((s,c)=>s+c.area,0),occ=1-white/total,missing=largestRatio<.16?(.16-largestRatio)*190:largestRatio>.58?(largestRatio-.58)*55:0,fragment=small.length*3.5+Math.max(0,comps.length-5)*2.5,sliver=slivers.length*8,ragged=largest.ragged*24,occupancy=occ>.8?(occ-.8)*145:occ<.34?(.34-occ)*55:0,cost=clamp(missing+fragment+sliver+ragged+occupancy-(largest.lat?7:0),0,100);return{cost,largestRatio,components:comps.length,smallCount:small.length,sliverCount:slivers.length,occ}}
function composition(ps,start,V){let a=ps.map(p=>({p,m:intersect(p,start,start+V)*p.span})).filter(x=>x.m>0).sort((x,y)=>y.m-x.m);if(!a.length)return 80;let c=0;if(a.length>3)c+=(a.length-3)*13;if(a.length>1){let r=a[0].m/a[1].m;if(r<1.22)c+=(1.22-r)*70}return clamp(c,0,100)}
function semanticCost(ps){let vals=[];for(let i=1;i<ps.length;i++){let e=entries[i],m=editorialValue(e);if(!m)continue;let p=ps[i],q=ps[i-1],gap=Math.max(0,p.row-(q.row+q.rows)),same=(p.x===q.x||p.x+p.span===q.x+q.span),c=0;if(m.continuity>.6){c+=Math.max(0,gap-5)*2*m.continuity;c+=same?0:10*m.continuity}if(m.topicShift>.7){c+=gap<5?(5-gap)*2*m.topicShift:0;c+=same?6*m.topicShift:0}if(m.dependency==='dependsOnPrevious'){c+=gap>6?(gap-6)*2:0;c+=same?0:10}vals.push(clamp(c,0,100))}return mean(vals)}
